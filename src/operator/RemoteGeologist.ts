...
interface RemoteGeologistMemory extends CreepMemory {
    targetRoom: string;
    target?: Id<any> | null;
}

export class RemoteGeologistCreep extends Creep {
    memory!: RemoteGeologistMemory;
...
export class RemoteGeologist extends Operator {

    creep: RemoteGeologistCreep | null;

    constructor(name: string, room: Room, targetRoom: string) {
        super(name, room);

        if (Game.creeps[this.name]) {
            this.creep = new RemoteGeologistCreep(Game.creeps[this.name].id, targetRoom);
        } else {
            this.creep = null;
        }
        this.room = room;
