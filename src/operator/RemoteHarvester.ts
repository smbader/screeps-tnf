...
export class RemoteHarvesterCreep extends Creep {
...
export class RemoteHarvester extends Operator {

    creep: RemoteHarvesterCreep | null;

    constructor(name: string, room: Room, targetRoom: string, assignedTarget: Id<any>) {
        super(name, room);

        if (Game.creeps[this.name]) {
            this.creep = new RemoteHarvesterCreep(Game.creeps[this.name].id, targetRoom, assignedTarget);
        } else {
            this.creep = null;
        }
        this.room = room;
        ...
        if (this.creep.memory.working == true) {
            ...
            if (this.creep.ticksToLive && this.creep.ticksToLive < 1400 &&
                ...
                if (spawn) {
                    ...
                    this.creep.travelTo(new RoomPosition(25, 25, this.creep.room.name));
                    this.creep.say('⛽')
                    return;
                }
            }
