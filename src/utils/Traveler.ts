...
export class Traveler {
    ...
    public findRoute(origin: string, destination: string, options: TravelToOptions = {}): {[roomName: string]: boolean } | false  {

        if (!options.restrictDistance) {
            options.restrictDistance = 16;
        }

        if (Game.map.getRoomLinearDistance(origin, destination) > options.restrictDistance) { return false; }
        ...
        let ret = Game.map.findRoute(origin, destination, {
            routeCallback: (roomName: string) => {
                ...
                if (Game.map.getRoomLinearDistance(origin, roomName) > 16) { return false; }
                let parsed;
    ...
    public findTravelPath(origin: {pos: RoomPosition}, destination: {pos: RoomPosition},
        ...
        if (options.useFindRoute || (!options.useFindRoute && Game.map.getRoomLinearDistance(origin.pos.roomName, destination.pos.roomName) > 2)) {
    ...
    public travelTo(creep: Creep, destination: {pos: RoomPosition}, options: TravelToOptions = {}):
        ...
        */
        ...
        if (!travelData.path || travelData.path.length === 0 || travelData.path == undefined) {
            console.log('nopath');
            return ERR_NO_PATH;
        }
