...
export class ConstructionCompany extends Operation {
    ...
    getSpawnRequests(): SpawnRequest[] {
        ...
        for (const operator of this.operationOperators) {
            // Only spawn if the operator indicates it's needed and creep doesn't exist
            if (operator.spawnIfNeeded && !Game.creeps[operator.name]) {
                const spawns = operator.room.find(FIND_MY_SPAWNS);
                if (spawns.length > 0) {
                    requests.push({
                        role: "ConstructionCrew",
                        name: operator.name,
                        ...
                        priority: 2, // Lower priority than energy management
                        room: operator.room
                    });
                }
            }
