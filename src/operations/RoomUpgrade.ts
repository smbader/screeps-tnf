    ...
    getSpawnRequests(): SpawnRequest[] {
        ...
        for (const operator of this.operationOperators) {
            ...
            if (!Game.creeps[operator.name]) {
                ...
                if (spawns.length > 0) {
                    // Different body parts based on operator type and room level
                    let body: BodyPartConstant[];
                    let priority: number;
                    
                    if (operator instanceof Harvester) {
                        body = [MOVE, WORK, WORK, WORK];
                        priority = 5; // Highest priority
                    } else {
                        // RoomUpgrader
                        if (operator.room.controller?.level == 1) {
                            body = [WORK, MOVE, CARRY, MOVE];
                            priority = 4; // High priority for level 1
                        } else {
                            body = [CARRY, MOVE, CARRY, MOVE];
                            priority = 1; // Lower priority for higher levels
                        }
                    }
                    
                    requests.push({
                        role: operator.constructor.name,
                        name: operator.name,
                        body: body,
                        memory: {
                            role: operator.constructor.name,
