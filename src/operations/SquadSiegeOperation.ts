...
import { CombatEvaluator } from "../utils/CombatEvaluator";
...
export class SquadSiegeOperation extends Operation {
  ...
  private getSquadCreepNames(): string[] {
    return SQUAD_ROLES.map(
      (role, i) => `Siege${role.charAt(0).toUpperCase() + role.slice(1)}_${this.targetRoom}_${this.activeSquadId}_${i}`
    );
  }
  ...
  private visualizeCreep(creep: Creep, text: string, color = "yellow", stroke = "black"): void {
    new RoomVisual(creep.room.name).text(text, creep.pos.x, creep.pos.y - 1.2, { color, stroke, font: 0.7 });
  }
  ...
  private setBoostsInRoomMemory(): void {
    ...
    this.logStatus(
      `Set Room.memory.data.labs.boosts to [${boostAssignments.map(b => `${b.component} in lab ${b.id}`).join(", ")}]`
    );
  }
  ...
  private findBestBreachWall(room: Room, fromPos: RoomPosition): Structure | null {
    ...
    if (bestTarget) {
      this.logStatus(
        `Selected breach wall/rampart at (${bestTarget.pos.x},${bestTarget.pos.y}) hits: ${bestTarget.hits}`
      );
  ...
  private findNearestExit(creep: Creep): RoomPosition | null {
    ...
    const retreatRoom = exits.find(
      e => e.x === closestExit!.x && e.y === closestExit!.y && e.roomName !== this.targetRoom
    )?.roomName;
  ...
  private healSquad(): void {
    ...
    for (const creep of creeps) {
      ...
      const injured = creeps.filter(c => c.hits < c.hitsMax).sort((a, b) => a.hits / a.hitsMax - b.hits / b.hitsMax)[0];
  ...
  private evaluateAttackWinnability(): boolean {
    const targetRoom = Game.rooms[this.targetRoom];
    if (!targetRoom) {
      this.logStatus("Cannot evaluate: target room not visible");
      return false;
    }

    const creeps = this.getLivingSquadCreeps();
    if (creeps.length === 0) {
      this.logStatus("Cannot evaluate: no squad members");
      return false;
    }

    // Only re-evaluate every 50 ticks to avoid excessive computation
    if (
      this.squadMemory.lastEvaluationTick &&
      Game.time - this.squadMemory.lastEvaluationTick < 50 &&
      this.squadMemory.attackWinnable !== undefined
    ) {
      return this.squadMemory.attackWinnable;
    }

    const evaluation = CombatEvaluator.evaluateAttack(creeps, targetRoom);

    this.squadMemory.lastEvaluationTick = Game.time;
    this.squadMemory.attackWinnable = evaluation.isWinnable;

    this.logStatus(
      `Attack Evaluation: ${evaluation.isWinnable ? "WINNABLE" : "NOT WINNABLE"} ` +
        `(confidence: ${(evaluation.confidence * 100).toFixed(0)}%, score: ${evaluation.advantageScore.toFixed(0)})`
    );

    for (const reason of evaluation.reasoning) {
      this.logStatus(`  - ${reason}`);
    }

    return evaluation.isWinnable;
  }
  ...
  private retreatSquad(): void {
    ...
    if (!this.squadMemory.retreatPos) {
      ...
      this.logStatus(
        `Squad retreating to (${this.squadMemory.retreatPos.x},${this.squadMemory.retreatPos.y},${this.squadMemory.retreatPos.roomName})`
      );
    }

    const retreatPos = new RoomPosition(
      this.squadMemory.retreatPos.x,
      this.squadMemory.retreatPos.y,
      this.squadMemory.retreatPos.roomName
    );
    const leader = creeps[0];
  ...
  private attackHostiles(): boolean {
    ...
    creeps.forEach(creep => {
      ...
      if (creep.hits < creep.hitsMax * 0.75 || squadNeedsHealing) {
        this.logStatus(
          `${creep.name} skipping attack due to low health (${creep.hits}/${creep.hitsMax}) or squad healing needed`
        );
        return;
      }
  ...
  private breachWall(): boolean {
    ...
    this.logStatus(
      `Breaching wall at (${this.wallTarget.pos.x},${this.wallTarget.pos.y}) hits: ${this.wallTarget.hits}`
    );
    ...
    creeps.forEach(creep => {
      ...
      if (creep.hits < creep.hitsMax * 0.75 || squadNeedsHealing) {
        this.logStatus(
          `${creep.name} skipping breach due to low health (${creep.hits}/${creep.hitsMax}) or squad healing needed`
        );
        if (creep !== leader) {
          creep.travelTo(leader.pos);
          this.visualizeCreep(creep, "FormUp", "gray");
  ...
  private handleKiting(creep: Creep, target: Creep): void {
    ...
    const range = creep.pos.getRangeTo(target);
    if (range <= 1) {
      const fleePath = PathFinder.search(creep.pos, [{ pos: target.pos, range: 2 }], {
        flee: true,
        maxRooms: 1,
        roomCallback: (roomName: string) => {
          const room = Game.rooms[roomName];
  ...
  actions(): void {
    ...
    // Evaluate if the attack is winnable once in the target room
    const creeps = this.getLivingSquadCreeps();
    if (creeps.length > 0 && creeps.every(c => c.room.name === this.targetRoom)) {
      const isWinnable = this.evaluateAttackWinnability();
      if (!isWinnable) {
        // Attack deemed not winnable - hold position and heal
        this.logStatus("Attack evaluation: NOT WINNABLE. Holding position.");
        this.healSquad();

        // If squad is fully healed, consider retreating
        if (this.allFullyHealed() && !this.squadMemory.retreating) {
          this.logStatus("Attack not winnable. Initiating strategic retreat.");
          this.squadMemory.retreating = true;
          delete this.squadMemory.retreatPos;
        }
        return;
